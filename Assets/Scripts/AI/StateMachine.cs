...
public class StateMachine : MonoBehaviour {
  ...
  public State SetNextState () {
    ...
    int current = currentState? currentState.transform.GetSiblingIndex(): -1;
    for (int i=1; i<transform.childCount; i++) {
      State possibleNext = transform.GetChild((current + i) % transform.childCount).GetComponent<State>();
      if (possibleNext) {
        possibleNext.gameObject.SetActive(true);
        return possibleNext;
      }
    }
